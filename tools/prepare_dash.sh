...
prepared_marker=${expected_sha}:patchset-12
...
patch -d "${temporary}" -p1 \
    < "${project_root}/ports/dash/patches/0006-recover-exhausted-redirections.patch"
patch -d "${temporary}" -p1 \
    < "${project_root}/ports/dash/patches/0007-report-job-admission-refusal.patch"
(
    cd "${temporary}"
